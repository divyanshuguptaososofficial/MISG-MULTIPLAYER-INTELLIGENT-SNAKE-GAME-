/* --------------Some Information-----------------
   ...
   Coordinates D:\harshit\divyanshu study\c\snakeai\main.cppof Time (40,21)      Player VS Computer
...
enum EDir2{ki2Down=115,ki2Left=97,ki2Right=100,ki2Up=119};
...
public:
       ...
       void showSingleBoundary () {
            ...
            cout<<" MULTIPLAYER INTELLIGENT SNAKE GAME";
            for (int i=0; i<10; i++) {
                cout<<'\xB0';
            }
      ...
      //Show Computer Vs computer
       void showCompVsCompBoundary () {
            cout<<"  ";
            HANDLE hConsole;
            hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
            SetConsoleTextAttribute(hConsole, 480);                          //Yellow Color Code=480
       ...
       void welcomeScreen () {
            ...
            SetConsoleTextAttribute(hConsole, 2);             //blue-Green Color Code=3
            gotoxy(5,22);
            cout<<"Play User vs User Game\n\n";

            SetConsoleTextAttribute(hConsole, 15);             //blue-Green Color Code=3
            gotoxy(2,23);
            cout<<"7: ";
            SetConsoleTextAttribute(hConsole, 2);             //blue-Green Color Code=3
            gotoxy(5,23);
            cout<<"Play Comp vs Comp\n\n";
...
public:
       ...
       //3 argument constructor for user 2 snake
        CSnake (int l,int cx,int cy)
       {
            iLength=l;
             iScore=0;
       ...
       //five rgument for omputer 2 snake
        CSnake (int l, int cx, int cy, int direction,int nouse)
       {
             iLength=l;
             iScore=0;
             for (int i=iLength; i>=0; i--)
...
char directionCheckComp2(char comp, CSnake s4, CFood f1)
{
          int dif_x = f1.x - s4.qaLoc[0].x;   //horizontal distance from food
          int dif_y = f1.y - s4.qaLoc[0].y;   //vertical distance from food
          //Going Right & Up (Food Forward)
          if (s4.cDir==kiRight && dif_x>0 && dif_y<0) {
              comp=kiRight;
          }
          if (s4.qaLoc[0].x == f1.x && comp==kiRight && dif_y<0) {
             comp=kiUp;
          }
          ...
          if (s4.qaLoc[0].x == f1.x && comp==kiLeft && dif_y<0) {
              comp=kiUp;
          }
          ...
          if (s4.qaLoc[0].y == f1.y && comp==kiUp && dif_x<0) {
             comp=kiLeft;
          }
          ...
          if (s4.qaLoc[0].y == f1.y && comp==kiDown && dif_x<0) {
             comp=kiLeft;
          }
...
int main ()
{
    ...
    do {
        ...
        CSnake s3(5,40,8);// user2 snake
        CSnake s2(5,40,8,75);  //Computer Snake
        CSnake s4(5,15,10,75,1);//computer 2
...
//___________________________________Show High Score Table______________________________//
    ...
    {               //START OF USER VS USER PLAYER GAME
USER_USER_GAME:
hit=0, available=0, user=0, welcome=0, esc=0;      //Reseting Old Variables
       count=0, temp=50;                                  //for 2 player game
       min=1, sec=59, timer=0;                            //time variables
       ...
       {          //START OF WHILE LOOP
          ...
          if (kbhit()) {                //If any key is pressed, then get that key
             ...
             if (cDoThis==kiUp || cDoThis==kiRight || cDoThis==kiDown || cDoThis==kiLeft ) {
              ...
              if (cDoThis==ki2Up || cDoThis==ki2Right || cDoThis==ki2Down || cDoThis==ki2Left ) {
                cTempDir=cDoThis;
             }
          }
          //User Snake is Going Down, chance of left or right
          ...
          //If user has pressed ESC, break while loop
          if(cDoThis==kiEscape) { hit=0; esc=1; break; }
            if(cDoThis=='p' || cDoThis=='P')
          {
              //================= Save User Snake =================
              ofstream outfile3("Snake4.txt" , ios::binary);
              if( !outfile3.good() ){
              cout<<"File could not be opened";
              }
              outfile3.write( reinterpret_cast<char*>(&s1), sizeof(s1) );
              outfile3.close();
              //================= Save Computer Snake =================
              ofstream outfile4("Snake5.txt" , ios::binary);
              if( !outfile4.good() ){
              cout<<"File could not be opened";
              }
              outfile4.write( reinterpret_cast<char*>(&s3), sizeof(s3) );
              outfile4.close();
              cDoThis=='~';
              getch();         //Stop the game
          }

         //User 2 Snake is Going Down, chance of left or right
          if(cTempDir==ki2Left && s3.cDir==ki2Down) {  s3.cDir=ki2Left; }
          if(cTempDir==ki2Right && s3.cDir==ki2Down) {  s3.cDir=ki2Right; }
          //User Snake is Going Left, chance of up or down
          if(cTempDir==ki2Down && s3.cDir==ki2Left) {  s3.cDir=ki2Down; }
          if(cTempDir==ki2Up && s3.cDir==ki2Left) {  s3.cDir=ki2Up; }
          //User Snake is Going Up, chance of left or right
          if(cTempDir==ki2Left && s3.cDir==ki2Up) {  s3.cDir=ki2Left; }
          if(cTempDir==ki2Right && s3.cDir==ki2Up) {  s3.cDir=ki2Right; }
          //User Snake is Going Right, chance of up or down
          if(cTempDir==ki2Up && s3.cDir==ki2Right) {  s3.cDir=ki2Up; }
          if(cTempDir==ki2Down && s3.cDir==ki2Right) {  s3.cDir=ki2Down; }
          //If user has pressed ESC, break while loop
          if(cDoThis==kiEscape) { hit=0; esc=1; break; }
        ...
        //Collision with Upper Horizontal Boundary for User2
          for (int i=0; i<b.iHori; i++) {
              if(s3.qaLoc[0]==b.qpUpBound[i]) {  hit=1; user=0; break; }
          }
          if (hit==1) { break; }
          //Collision with Right Vertical Boundary for User2
          for (int i=0; i<b.iVirt; i++) {
               if(s3.qaLoc[0]==b.qpRightBound[i]) {  hit=1; user=0; break;}
          }
          if (hit==1) { break; }
          //Collision with Lower Horizontal Boundary for User2
          for (int i=0; i<b.iHori; i++) {
              if(s3.qaLoc[0]==b.qpDownBound[i]) {  hit=1; user=0; break; }
          }
          if (hit==1) { break; }
          //Collision with Left Vertical Boundary for User2
          for (int i=0; i<b.iVirt; i++) {
              if(s3.qaLoc[0]==b.qpLeftBound[i]) {  hit=1; user=0; break; }
          }
          ...
          if (hit==1) { break; }
               ...
               //Self Collision for User2
          for (int i=1; i<s1.iLength; i++) {
              if (s3.qaLoc[0] == s3.qaLoc[i]) { hit=1; user=0; break; }
          }
          if (hit==1) { break; }
          ...
          if (s1.qaLoc[0] == f1.qEatMe )
          {
           ...
           //Eating Food for User2
          if (s3.qaLoc[0] == f1.qEatMe )
          {
             s3++;
             SetConsoleTextAttribute(hConsole, 13);                    //Purple Color Code = 13
             f1.generateFood();
             SetConsoleTextAttribute(hConsole, 11);                    //light blue Color Code = 10
             s3.printScore(66,21);
             count++;
             if (count==5) { g1.generateBigFood(); available=1; count=0; }
          }
          ...
          //Eating Big Food for User3
          if (s3.qaLoc[0] == g1.qEatBigFood )
          {
             int randNum =(rand() % 50);
             s3.iScore = s3.iScore + randNum;
             s3.printScore(16,21);
             b.hideTime();
             available=0;
             temp=50;
          ...
          for (int j=1; j<s1.iLength; j++)
          {
           ...
           //If food is generated on body of user2 snake,
          //clear it and generate new food
          for (int j=1; j<s3.iLength; j++)
          {
             if (f1.qEatMe == s3.qaLoc[j]) { f1.generateFood();  }
          }
       }          //END OF WHILE LOOP-------------------------------------
       //Check how game is ended
       if (hit==1) {       //by collision with anything
          if (user==0) {         //If user2 has won game
             Sleep(1200);
             system("cls");
             b.gameOverScreen();
             gotoxy(35,9);
          ...
          else if (user==1) {                  //If user1 has won the game
             Sleep(1200);
             system("cls");
             b.gameOverScreen();
             gotoxy(31,9);
          ...
          system("cls");
            ...
            gotoxy(35,9);
            cout<<"Time Over!";
            if (s1.iScore>s3.iScore) {     //If User's score is more
               gotoxy(36,11);
               SetConsoleTextAttribute(hConsole, 10);                    //Green Color Code = 10
               cout<<"User 1 Won!";
               Sleep(1200);
               ...
               gotoxy(36,11);             //If user 2 score is more
               SetConsoleTextAttribute(hConsole, 13);                    //Purple Color Code = 13
               cout<<"User 2 Won!";
               gotoxy(30,23);
               Sleep(1200);
    ...
    {                    //START OF Computer VS COMPUTER GAME
COMP_VS_COMP_GAME:
       hit=0, available=0, user=0, welcome=0, esc=0;      //Reseting Old Variables
       count=0, temp=50;                                  //for 2 player game
       min=1, sec=59, timer=0;                            //time variables
       ...
       b.setDownBoundary();
       b.setUpBoundary();
       b.showCompVsCompBoundary();
       //User Snake
       SetConsoleTextAttribute(hConsole, 10);                    //Green Color Code = 10
       s4.show();                        //Showing User Snake
       s4.printScore(16,21);
       //Computer Snake
       SetConsoleTextAttribute(hConsole, 11);                    //Light Blue Color Code = 11
       s2.show();                        //Showing Computer Snake
       ...
       {          //START OF WHILE LOOP
          ...
          //Check in which direction, computer has to go
          s2.cDir = directionCheckComp(s2.cDir,s2,f1);
               s4.cDir = directionCheckComp2(s4.cDir,s4,f1);
          //Boundaries for User
          ...
          //Collision with Upper Horizontal Boundary for Computer
          for (int i=0; i<b.iHori; i++) {
              if(s4.qaLoc[0]==b.qpComUpBound[i] && s4.cDir==kiUp) {
                  if (i>=0 && i<70) { s4.cDir=kiRight; }   //If it has come on any boundary
                  else s4.cDir=kiLeft;                     //turn the direction
              }
          }
          if (hit==1) { break; }
          //Collision with Right Vertical Boundary for Computer
          for (int i=0; i<b.iVirt; i++) {
               if(s4.qaLoc[0]==b.qpComRightBound[i] && s4.cDir==kiRight) {
                  if (i>=0 && i<8) { s4.cDir=kiDown; }
                  else s4.cDir=kiUp;
              }
          }
          if (hit==1) { break; }
          //Collision with Lower Horizontal Boundary for Computer
          for (int i=0; i<b.iHori; i++) {
              if(s4.qaLoc[0]==b.qpComDownBound[i] && s4.cDir==kiDown) {
                  if (i>=0 && i<36) { s4.cDir=kiRight; }
                  else s4.cDir=kiLeft;
              }
          }
          if (hit==1) { break; }
          //Collision with Left Vertical Boundary for Computer
          for (int i=0; i<b.iVirt; i++) {
              if(s4.qaLoc[0]==b.qpComLeftBound[i] && s4.cDir==kiLeft) {
                  if (i>=0 && i<8) { s4.cDir=kiDown; }
                  else s4.cDir=kiUp;
              }
          }
          ...
          for (int i=1; i<s2.iLength; i++) {
           ...
           //Self Collision for Computer
          for (int i=1; i<s4.iLength; i++) {
              if (s4.qaLoc[0] == s4.qaLoc[i]) { hit=1; user=1; break; }
          }
          if (hit==1) { break; }

          //Eating Food for Computer
                   if (s4.qaLoc[0] == f1.qEatMe )
          {
             s4++;
             SetConsoleTextAttribute(hConsole, 13);                    //Purple Color Code = 13
             f1.generateFood();
          ...
          for (int j=1; j<s2.iLength; j++)
          {
            ...
            //If food is generated on body of computer snake,
          //clear it and generate new food
          for (int j=1; j<s4.iLength; j++)
          {
             if (f1.qEatMe == s4.qaLoc[j]) { f1.generateFood();  }
          }
       }          //END OF WHILE LOOP-------------------------------------
       //Check how game is ended
       if (hit==1) {       //by collision with anything
          ...
          system("cls");
            ...
            gotoxy(35,9);
            cout<<"Time Over!";
            if (s4.iScore>s2.iScore) {     //If User's score is more
               gotoxy(36,11);
               SetConsoleTextAttribute(hConsole, 10);                    //Green Color Code = 10
               cout<<"Alexa Won!";
               Sleep(1200);
           ...
           //END OF Comp VS COMPUTER GAME

           //______________________Play Saved Player VS Player Game___________________________//
    else if(choice=='8')  {
         //Loading User Snake for Player VS Computer Game
         ifstream infile3("Snake4.txt", ios::binary);
         if( !infile3.good() )  {
              cout<<"File could not be opened";
          }
          //Loading Computer Snake for Player VS Computer Game
         infile3.read( reinterpret_cast<char*>(&s1), sizeof(s1) );
         ifstream infile4("Snake5.txt", ios::binary);
         if( !infile4.good() )  {
              cout<<"File could not be opened";
          }
         infile4.read( reinterpret_cast<char*>(&s3), sizeof(s3) );
         goto USER_USER_GAME;          //Start Game
    }
//__________________________________Exit the Game____________________________________//
    else if (choice=='9') {
         exit(1);
    }
    b.welcomeScreen();
